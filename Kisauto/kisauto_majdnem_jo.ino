...
#define LOW_SPEED 20
#define LONG_DELAY_TIME 50 
#define DELAY_TIME 50
#define SHORT_DELAY_TIME 20
...
void init_servo()
{
  servo.attach(4); // szervó jelvezetéke a D4-re van kötve (s --> 4)
}
...
void loop(){
  ...
  if (currentMillis - lastUltrasPing >= ultrasInterval) {
    lastUltrasPing = currentMillis;
    trigpin();  // ultrahang mérése időzítve
  }

  if (currentMillis - lastTracking >= trackingInterval) {
    lastTracking = currentMillis;
    tracking(); // sorkövetés időzítve
  }
